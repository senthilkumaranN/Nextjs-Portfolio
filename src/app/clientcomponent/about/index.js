...
import aboutMeImage from "../../assets/student.jpeg";
...
export default function ClientAboutView({ data }) {
    ...
    const aboutDataInfo = [
        {
            label: "Client",
            value: aboutData?.noofclients || "0",
        },
        {
            label: "Projects",
            value: aboutData?.noofprojects || "0",
        },
        {
            label: "Experience",
            value: aboutData?.yearofexperience || "0",
        },
    ];
        ...
        <div className="max-w-screen-xl mt-24 mb-6 sm:mt-14 sm:mb-14 px-6 sm:px-8 lg:px-16 mx-auto" id="about">
            ...
            <AnimationWrapper className={"pt-6"}>
                <div className="flex flex-col justify-center items-center row-start-2 sm:row-start-1">
                    ...
                    <p className="text-[#000] mt-4 mb-8 font-bold">{aboutData?.aboutme}</p>
                </div>
            </AnimationWrapper>
            <div className="grid grid-flow-row sm:grid-flow-col grid-cols-1 sm:grid-cols-2 gap-8">
                ...
                <AnimationWrapper className={"flex items-center w-full p-4"}>
                    ...
                    >
                        {(aboutData?.skills || "").split(",").map((skill) => 
                        skill.trim()).filter((skill)=> skill !=="").map((skill,index)=> (
                            <motion.div key={index}
                                className="w-full flex justify-center items-center"
                                variants={skillItemVariant}
                            >
                                <button className="whitespace-nowrap text-ellipsis overflow-hidden capitalize
                                 py-3 w-[160px] px-6 border-[2px] border-green-main bg-[#fff] text-[#000]
                                  font-semibold rounded-lg text-xl tracking-widest hover:shadow-green-main transition-all outline-none">
                                    {skill}
                                </button>
                            </motion.div>
                        ))}
