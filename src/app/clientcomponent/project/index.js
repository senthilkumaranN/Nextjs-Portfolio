      ...
      <AnimationWrapper>
        <ul className="project-wrapper" ref={containerRef}>
          {data && data.length
            ? data.map((item, index) => (
              <li
                className="w-full min-h-screen flex items-stretch cursor-pointer"
                key={item._id}
              >
                <div className="border-2 w-full relative border-green-main transition-all rounded-lg flex flex-col">
                      ...
                      <div className="flex flex-col">
                        ...
                        <div className="grid gap-2 mt-5 grid-cols-2 h-full max-h-[200px] w-full">
                          {item?.technologies.split(",").map((techItem) => (
                            <div className="w-full flex justify-start items-center" key={techItem._id}>
                        ...
                        <div className="flex flex-col">
                          <p className="text-[15px] text-black-500 uppercase font-semibold pt-[8px]">
                            {item.summary}
                          </p>
                        </div>
                      </div>
                  ...
                  <div className="absolute w-full bottom-0 justify-center flex gap-2">
                    <button onClick={() => router.push(item.website)} className="p-2 text-white-500 font-semibold text-[14px] tracking-widest bg-green-main transition-all outline-none rounded-md">
                      Website
                    </button>
                    <button onClick={() => router.push(item.github)} className="p-2 text-white-500 font-semibold text-[14px] tracking-widest bg-green-main transition-all outline-none rounded-md">
                      Github
                    </button>
                  </div>
                </div>
